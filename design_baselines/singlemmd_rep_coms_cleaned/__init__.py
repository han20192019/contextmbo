...
from design_baselines.singlemmd_rep_coms_cleaned.trainers import ConservativeObjectiveModel
from design_baselines.singlemmd_rep_coms_cleaned.nets import ForwardModel
from design_baselines.singlemmd_rep_coms_cleaned.nets import RepModel
...
def coms_cleaned(
    ...
    for step in range(0, 1 + particle_evaluate_gradient_steps):
        ...
        if (not fast and step%50==0) or step == particle_evaluate_gradient_steps:
            solution = xt
            ...
            mmd = tf.keras.losses.mean_squared_error(group_logged_rep, xt_rep)
            logger.record(f"solver/mmd",
                            mmd, step+1, percentile=True)
