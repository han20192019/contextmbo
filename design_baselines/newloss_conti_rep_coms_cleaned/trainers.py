...
class ConservativeObjectiveModel(tf.Module):
    ...
    def optimize(self, x, steps, **kwargs):
        ...
        def gradient_step(xt):
            with tf.GradientTape() as tape:
                ...
                #new loss
                dataset_rep = self.rep_model(self.datax, training = False)
                dataset_rep_mean = tf.reduce_mean(dataset_rep, axis=0)
                learned_rep = tf.reduce_mean(xt_rep, axis=0)
                mmd = tf.reduce_mean(tf.keras.losses.mean_squared_error(learned_rep, dataset_rep_mean))

                # the conservatism of the current set of particles
                loss = self.entropy_coefficient * entropy + score - 5000*mmd
    ...
    def train_step(self, x, y):
        ...
        x_sort = tf.gather(x, tf.argsort(y,axis=0), axis=0)
        x_sort = tf.reshape(x_sort,[x_sort.shape[0], x_sort.shape[2]])
        x_top = x_sort[:int(x_sort.shape[0]*0.2)]
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            rep_x_top = self.rep_model(x_top, training= False)
            logged_rep_top = tf.reduce_mean(rep_x_top, axis=0)
            learned_rep = tf.reduce_mean(rep_x_neg, axis=0)
            mmd = tf.reduce_mean(tf.keras.losses.mean_squared_error(learned_rep, logged_rep_top))
