...
class ConservativeObjectiveModel(tf.Module):
    ...
    def __init__(self, discriminator_model, mmd_param, policy_model, policy_model_lr, rep_model, rep_model_lr, 
        ...
        super().__init__()
        discriminator_model_opt = tf.keras.optimizers.Adam
        ...
        self.discriminator_model_opt = \
            discriminator_model_opt(learning_rate=rep_model_lr)
    ...
    def train_step(self, x, y):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            print(output_shape)
            valid = tf.constant([1.0],shape=[output_shape[0]])
            fake = tf.constant([0.0],shape=[output_shape[0]])
            real_loss = self.adversarial_loss(self.discriminator_model(rep_x, training=True), valid)
            fake_loss = self.adversarial_loss(self.discriminator_model(rep_x_neg, training=True), fake)
        ...
        dis_grads = tape.gradient(
            d_loss, self.discriminator_model.trainable_variables)
        ...
        self.discriminator_model_opt.apply_gradients(zip(
            dis_grads, self.discriminator_model.trainable_variables))
