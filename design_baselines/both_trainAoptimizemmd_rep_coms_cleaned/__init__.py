...
from design_baselines.both_trainAoptimizemmd_rep_coms_cleaned.trainers import ConservativeObjectiveModel
from design_baselines.both_trainAoptimizemmd_rep_coms_cleaned.nets import ForwardModel
from design_baselines.both_trainAoptimizemmd_rep_coms_cleaned.nets import RepModel
...
def coms_cleaned(
    ...
    for step in range(0, 1 + particle_evaluate_gradient_steps):
        ...
        if not fast or step == particle_evaluate_gradient_steps:
            ...
            logger.record(f"solver/mmd_100th",mmd_scoresort[-1], step+1)
            logger.record(f"solver/mmd_90th",mmd_scoresort[int(mmd_scoresort.shape[0]*0.9)], step+1)
            logger.record(f"solver/mmd_50th",mmd_scoresort[int(mmd_scoresort.shape[0]*0.5)], step+1)
            #logger.record(f"solver/mmd",mmd, step+1, percentile=True)
            mmd_mean = tf.reduce_mean(mmd)
            logger.record(f"solver/mmd_mean",
                            mmd_mean, step+1, percentile=True)

            dif = prediction - mmd_param*tf.sqrt(mmd+1e-6)
            ...
            logger.record(f"solver/dif_50th",dif_scoresort[int(dif_scoresort.shape[0]*0.5)], step+1)

            dif_mean = tf.reduce_mean(dif)
            logger.record(f"solver/dif_mean",
                            dif_mean, step+1, percentile=True)

            dif = prediction - mmd_param*mmd
            dif_scoresort = tf.gather(dif, ids)
            logger.record(f"solver/square_dif_100th",dif_scoresort[-1], step+1)
            logger.record(f"solver/square_dif_90th",dif_scoresort[int(dif_scoresort.shape[0]*0.9)], step+1)
            logger.record(f"solver/square_dif_50th",dif_scoresort[int(dif_scoresort.shape[0]*0.5)], step+1)

            dif_mean = tf.reduce_mean(dif)
            logger.record(f"solver/square_dif_mean",
                            dif_mean, step+1, percentile=True)
