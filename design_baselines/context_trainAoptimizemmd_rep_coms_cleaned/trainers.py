...
class ConservativeObjectiveModel(tf.Module):
    ...
    def __init__(self, mmd_param, rep_model1, rep_model2, rep_model_lr, 
                 forward_model,
                 rep_model_opt1=tf.keras.optimizers.Adam,
                 rep_model_opt2=tf.keras.optimizers.Adam,
        ...
        self.rep_model1 = rep_model1
        self.rep_model2 = rep_model2
        self.rep_model_lr = rep_model_lr
        self.forward_model = forward_model
        self.forward_model_opt = \
            forward_model_opt(learning_rate=forward_model_lr)
        self.rep_model_opt1 = \
            rep_model_opt1(learning_rate=rep_model_lr)
        self.rep_model_opt2 = \
            rep_model_opt2(learning_rate=rep_model_lr)
    ...
    def optimize(self, flag, c, x, steps, **kwargs):
        ...
        def gradient_step(xt):
            with tf.GradientTape() as tape:
                ...
                rep_x_first = self.rep_model1(x, training= False)
                pair = tf.concat([rep_x_first,c],1)
                xt_rep = self.rep_model2(pair, training= False)
    ...
    def train_step(self, c, x, y):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            rep_x_first = self.rep_model1(x, training= True)
            pair = tf.concat([rep_x_first,c],1)
            rep_x = self.rep_model2(pair, training= True)
            ...
            for i in range(2):
                c_here = np.zeros(8)
                c_here[i]=1
                indices1 = tf.where(c[:,i]==1)
                indices1 = tf.transpose(indices1)[0]
                x1 = tf.gather(x, indices1, axis=0)
                c1 = tf.gather(c, indices1, axis=0)
                c_neg_list.append(c1)
                x_neg_list.append(self.optimize(False, c1, 
                    x1, self.particle_gradient_steps, training=False))

            x_neg = tf.concat(x_neg_list,0)
            c_neg = tf.concat(c_neg_list,0)

            # calculate the prediction error and accuracy of the model
            rep_x_first_neg = self.rep_model1(x_neg, training=False)
            pair_neg = tf.concat([rep_x_first_neg,c_neg],1)
            rep_x_neg = self.rep_model2(pair_neg, training=False)
            ...
            model_loss3 = mse + mmd*mmd_param
            total_loss3 = tf.reduce_mean(model_loss3)
            statistics[f'train/loss3'] = total_loss3
        ...
        rep_grads1 = tape.gradient(
            total_loss2, self.rep_model1.trainable_variables)
        rep_grads2 = tape.gradient(
            total_loss3, self.rep_model2.trainable_variables)
        ...
        self.rep_model_opt1.apply_gradients(zip(
            rep_grads1, self.rep_model1.trainable_variables))
        self.rep_model_opt2.apply_gradients(zip(
            rep_grads2, self.rep_model2.trainable_variables))
    ...
    def train(self, dataset):
        ...
        for c, x, y in dataset:
            for name, tensor in self.train_step(c, x, y).items():
