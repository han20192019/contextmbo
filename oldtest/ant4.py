...
seed = [1,2,3,4,5]
mmd_param = [100]
d_list = [16, 32, 64, 128, 256] #[16, 32, 64, 128, 256]
name = "ant"
task = "AntMorphology-Exact-v0"
for d in d_list:
    for s in seed:
        for m in mmd_param:
            coms_cleaned(logging_dir = "/nfs/kun2/users/hanqi2019/1219ant300/seed"+str(s)+"/"+name+"rep"+str(d)+"seed"+str(s)+"mmd"+str(m),
                                    task=task,
                                    task_relabel=True,
                                    normalize_ys=True,
                                    normalize_xs=True,
                                    ...
                                    forward_model_activations=['relu', 'relu', 'relu', 'relu'],
