...
from design_baselines.all_both_trainAoptimizemmd_rep_coms_cleaned import coms_cleaned
seed = [1]
mmd_param = [10] #[0.1, 0, 10, 500, 2000, 5000, 10000]
...
for d in d_list:
    for s in seed:
        for m in mmd_param:
            coms_cleaned(logging_dir = "0121allbothtrainAoptimizemmd/"+name+"/opt"+str(opt)+"/seed"+str(s)+"/"+name+"rep"+str(d)+"seed"+str(s)+"mmd"+str(m),
                                    task=task,
                                    task_relabel=True,
                                    normalize_ys=True,
                                    normalize_xs=True,
